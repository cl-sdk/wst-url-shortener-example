...
(defun create-short-url-handler (request response)
  (let* ((raw-url (request-url request))
    ...
    (if (null target-url)
	(write-response response :status 400 :content "url field is required and cannot be empty")
  ...
  (build-webserver
   `(group
     ...
     (resource "/api/v1"
		...
		:route (route :POST create-short-url "/links" create-short-url-handler))
	       (route :GET list-short-urls "/links" list-short-urls-handler)
	       (route :GET inspect-short-url "/links/:code" inspect-short-url-handler)
	       (route :DELETE delete-short-url "/links/:code" delete-short-url-handler))
